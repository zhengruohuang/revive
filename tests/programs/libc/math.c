...
int ctz64(u64 val)
{
    ...
    for (int i = 0; i < 64 && !(val & 0x1); i++, val >>= 1) {
        trail_zeros++;
    }
...
int ctz32(u32 val)
{
    ...
    for (int i = 0; i < 32 && !(val & 0x1); i++, val >>= 1) {
        trail_zeros++;
    }
...
int ctz16(u16 val)
{
    ...
    for (int i = 0; i < 16 && !(val & 0x1); i++, val >>= 1) {
        trail_zeros++;
    }
...
{
#if (ARCH_WIDTH == 64)
    return (ulong)ctz64((u64)val);
#elif (ARCH_WIDTH == 32)
    return (ulong)ctz32((u32)val);
#elif (ARCH_WIDTH == 16)
    return (ulong)ctz16((u16)val);
#else
    #error Unsupported ARCH_WIDTH
#endif
}
